...
class EncoderLSTM_v2(nn.Module):
    ...
    def forward(self, x, vector_input=False):
        # Embed text
        x = self.embedding(x)
        x = self.dropout(x)
...
class Discriminator(nn.Module):
    def __init__(self, latent_dim = 1200, hidden_dim = 1024):
...
class VAE(nn.Module):
    def __init__(self, embedding_e1, embedding_e2, embedding_d1, embedding_d2, h_dim, num_layers, dropout_p, bi, attn_type, tokens_bos_eos_pad_unk=[0,1,2,3], reverse_input=False):
        ...
        self.encoder = VAEEncoderLSTM(embedding_e1, embedding_e2, h_dim, num_layers, dropout_p=dropout_p, bidirectional=bi)
    ...
    def reparameterize(self, mean, logvar):
        ...
        e = Variable(sd.data.new(sd.size()).normal_(),requires_grad=False)
    ...
    def forward(self, src, trg, lang_in, lang_out, vector_input = False):
        ...
        # Encode
        out_e, mean, logvar = self.encoder(src, lang_in, vector_input)
        # Reparameterize
        z = self.reparameterize(mean, logvar)
        # Pair with blank cell state
        state = (z, Variable(torch.zeros(z.shape)).cuda())
        # Decode
        out_d, final_d = self.decoder(trg, state, lang_out)
        # Attend
    ...
    def greedy(self, sentences, lang_in, lang_out, max_len=30, train=False):
        ...
        outputs_e, means, logvars = self.encoder(sentences, lang_in)
        ...
        while len(pending) > 0:
            ...
            # Pair means with new cell state
            states = (means, Variable(torch.zeros(means.shape)).cuda())
    ...
    def beam_search(self, src, beam_size,lang_in, lang_out, max_len, remove_tokens=[]):
        ...
        if self.reverse_input:
            ...
            src = src.index_select(0, inv_index)
        # Encode
        outputs_e, means, logvars = self.encoder(src, lang_in)
        # Start with '<s>'
        init_lprob = -1e10
        init_sent = [self.bos_token]
        best_options = [(init_lprob, init_sent, (means, Variable(torch.zeros(means.shape)).cuda()))] # beam
        # Beam search
...
class Denoise_AE_v2(nn.Module):
    def __init__(self, embedding, embedding_d1, embedding_d2, h_dim, num_layers, dropout_p, bi, attn_type, tokens_bos_eos_pad_unk=[0,1,2,3], reverse_input=False):
        super(Denoise_AE_v2, self).__init__()
        # Store hyperparameters
        self.h_dim = h_dim
        self.vocab_size, self.emb_dim = embedding.size()
        self.vocab_size_d1, self.emb_dim_d1 = embedding_d1.size()
        self.vocab_size_d2, self.emb_dim_d2 = embedding_d2.size()
        self.bos_token = tokens_bos_eos_pad_unk[0]
        self.eos_token = tokens_bos_eos_pad_unk[1]
        self.pad_token = tokens_bos_eos_pad_unk[2]
        self.unk_token = tokens_bos_eos_pad_unk[3]
        self.reverse_input = reverse_input
        # Create encoder, decoder, attention
        self.encoder = EncoderLSTM_v2(embedding, h_dim, num_layers, dropout_p=dropout_p, bidirectional=bi)
        self.decoder = DecoderLSTM(embedding_d1, embedding_d2, h_dim, num_layers * 2 if bi else num_layers, dropout_p=dropout_p)
        self.attention = Attention(pad_token=self.pad_token, bidirectional=bi, attn_type=attn_type, h_dim=self.h_dim)
        # Create linear layers to combine context and hidden state
        self.linear_com_s1 = nn.Linear(2 * self.h_dim, h_dim)
        ...
        self.tanh = nn.Tanh()
        self.dropout = nn.Dropout(dropout_p)
        self.linear_out_s1 = nn.Linear(h_dim, self.vocab_size_d1)
        self.linear_out_s2 = nn.Linear(h_dim, self.vocab_size_d2)
        # Tie weights of decoder embedding and output
        self.linear_out_s1.weight = self.decoder.embedding_s1.weight
        self.linear_out_s2.weight = self.decoder.embedding_s2.weight

    def forward(self, src, trg, lang_out, vector_input = False):
        if use_gpu: src = src.cuda()
        # Reverse src tensor
        if self.reverse_input:
            inv_index = torch.arange(src.size(0)-1, -1, -1).long()
            ...
            src = src.index_select(0, inv_index)
        # Encode
        out_e, final_e = self.encoder(src, vector_input)
        # Decode
        out_d, final_d = self.decoder(trg, final_e, lang_out)
        # Attend
        context = self.attention(src, out_e, out_d)
    ...
    def greedy(self, sentences, lang_in, lang_out, max_len=30, train=False):
        if use_gpu: entences = sentences.cuda()
        num_sent, batch_size = sentences.size()
        outputs_e, states = self.encoder(sentences)
        translations = [[self.bos_token] for i in range(batch_size)]
        prev_words = batch_size *[self.bos_token]
        pending = set(range(batch_size))
        output = Variable(torch.zeros(batch_size, self.decoder.h_dim), requires_grad=False)
    ...
    def predict(self, src, lang_out, beam_size=1):
        '''Predict top 1 sentence using beam search. Note that beam_size=1 is greedy search.'''
        beam_outputs = self.beam_search(src, beam_size, lang_out, max_len=30) # returns top beam_size options (as list of tuples)
        top1 = beam_outputs[0][1] # a list of word indices (as ints)
        return top1

    def predict_k(self, src, k, lang_out, max_len=30, remove_tokens=[]):
        '''Predict top k possibilities for first max_len words.'''
        beam_outputs = self.beam_search(src, k, lang_out, max_len=max_len, remove_tokens=remove_tokens) # returns top k options (as list of tuples)
        topk = [option[1] for option in beam_outputs] # list of k lists of word indices (as ints)
        return topk

    def beam_search(self, src, beam_size,lang_out, max_len, remove_tokens=[]):
        '''Returns top beam_size sentences using beam search. Works only when src has batch size 1.'''
        if use_gpu: src = src.cuda()
        # Reverse src tensor
        if self.reverse_input:
    ...
    def get_attn_dist(self, src, trg, lang_out):
        '''Runs forward pass, also returns attention distribution'''
        if use_gpu: src = src.cuda()
        # Reverse src tensor
        if self.reverse_input:
