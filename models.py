...
class VAEEncoderLSTM(EncoderLSTM):
    def __init__(self, embedding_s1, embedding_s2, h_dim, num_layers, dropout_p=0.0, bidirectional=True):
        
        #initialize base encoder
        super(VAEEncoderLSTM,self).__init__(embedding_s1, embedding_s2, h_dim, num_layers, dropout_p, bidirectional)
        
        #initialize layers to give mean and logvar
        self.l1_mean = nn.Linear(h_dim,h_dim)
        self.l1_logvar = nn.Linear(h_dim,h_dim)
        
    def forward(self, x, lang_in, vector_input=False):
        
        #run through base encoder
        out, h = super(VAEEncoderLSTM,self).forward(x, lang_in, vector_input)
        
        #calc mean and logvar and return
        mean = self.l1_mean(h[0])
        logvar = self.l1_logvar(h[0])
