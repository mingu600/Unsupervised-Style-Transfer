...
def train(train_iter_pos, train_iter_neg, val_iter_pos, val_iter_neg, model,discrim, recons_pos, recons_neg,d_loss,\
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(num_batches):
            ...
            z_hat_pos = discrim(model.encoder(pos_text, 0)[0].cpu()).cuda()
            z_pos = Variable(torch.ones(pos_text.size()[1])).cuda()
            z_hat_neg = discrim(model.encoder(neg_text, 0)[0].cpu()).cuda()
            ...
            z_hat_neg_2 = discrim(model.encoder(neg_text, 0)[0].cpu()).cuda()
